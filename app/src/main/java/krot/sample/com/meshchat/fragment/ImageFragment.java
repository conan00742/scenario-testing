    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_LOAD_IMAGE && resultCode == RESULT_OK && data != null) {
            ...
            for (int i = 0; i < HypeRepository.getRepository().getInstanceList().size(); i++) {
                ...
                Hype.send(imageByte, currentInstance, true);
            }

            UserMessage imgMsg = new UserMessage(msgData, true);
            DisplayedMessage displayedImageMsg = new DisplayedMessage(Hype.getHostInstance(), imgMsg);
            HypeRepository.getRepository().getImageMessageList().add(displayedImageMsg);
